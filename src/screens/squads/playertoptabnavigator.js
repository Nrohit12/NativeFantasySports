...
export default function TopTabnavigator() {
  ...
  const wicket_keeper = players.filter(player => player.role === "Wicket-Keeper")
  const bowler = players.filter(player => player.role === "Bowler")
  const all_rounder = players.filter(player => player.role === "All-Rounder")
  ...
  return (
    ...
    >
      <Tab.Screen
        ...
        initialParams={{players: batsman}}
      />
      <Tab.Screen
        name="WK"
        component={Playerlist}
        options={{ tabBarLabel: 'WK' }}
        initialParams={{players: wicket_keeper}}
      />
      <Tab.Screen
        name="AR"
        component={Playerlist}
        options={{ tabBarLabel: 'AR' }}
        initialParams={{players: all_rounder}}
      />
      <Tab.Screen
        name="BOWL"
        component={Playerlist}
        options={{ tabBarLabel: 'BOWL' }}
        initialParams={{players: bowler}}
      />
    </Tab.Navigator>
  );
}